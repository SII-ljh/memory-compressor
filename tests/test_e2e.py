...
def test_e2e_full_model_inference():
    """Test full pipeline: Full Model (RoPE + bias), inference."""
    ...
    print(f"[PASS] test_e2e_full_model_inference: logits={result['logits'].shape}")


def test_e2e_full_model_training():
    """Test full pipeline: Full Model, training with QA data."""
    cfg = _make_config(rope=True, bias=True)
    model = QCPC(cfg)
    model.set_stage(2)
    ...
    # In stage 2, QueryMapper and alpha should also get gradients
    # (if alpha has been updated to non-zero; at init alpha=0 so grad may be zero but should exist)
    assert model.perceiver.latent.z_base.grad is not None
    print(f"[PASS] test_e2e_full_model_training: loss={result['loss'].item():.4f}")


def test_e2e_all_four_combos():
    """Test all 4 config combinations in training mode."""
    combos = [
    ...
    print("[PASS] test_e2e_all_four_combos")
...
if __name__ == "__main__":
    ...
    test_e2e_full_model_inference()
    test_e2e_full_model_training()
    test_e2e_all_four_combos()
    test_e2e_with_masks()
